...
describe('Trade', () => {
  ...
  const pair_bapx_0 = new Pair(
    new TokenAmount(BAPx[ChainId.MAINNET], JSBI.BigInt(1000)),
  ...
  it('can be constructed with ETHER as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_bapx_0], ETHER, token0),
  ...
  it('can be constructed with ETHER as output', () => {
    const trade = new Trade(
      new Route([pair_bapx_0], token0, ETHER),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency input', () => {
      const result = Trade.bestTradeExactIn(
        [pair_bapx_0, pair_0_1, pair_0_3, pair_1_3],
        CurrencyAmount.ether(JSBI.BigInt(100)),
        token3
      )
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(ETHER)
      expect(result[0].route.path).toEqual([BAPx[ChainId.MAINNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(ETHER)
      expect(result[1].route.path).toEqual([BAPx[ChainId.MAINNET], token0, token3])
    ...
    it('works for ETHER currency output', () => {
      ...
      expect(result[0].route.path).toEqual([token3, token0, BAPx[ChainId.MAINNET]])
      expect(result[0].outputAmount.currency).toEqual(ETHER)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, BAPx[ChainId.MAINNET]])
