...
describe('Route', () => {
  ...
  const weth = BAPx[ChainId.MAINNET]
  const pair_0_1 = new Pair(new TokenAmount(token0, '100'), new TokenAmount(token1, '200'))
  const pair_0_bapx = new Pair(new TokenAmount(token0, '100'), new TokenAmount(weth, '100'))
  const pair_1_bapx = new Pair(new TokenAmount(token1, '175'), new TokenAmount(weth, '100'))
